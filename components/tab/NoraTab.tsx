...
const MemoNoraView = memo(
  ({ tab, ...props }: any) => {
    return <NoraView {...props} />
  },
  (props1, props2) => props1.tab.id == props2.tab.id,
)

export const NoraTab: React.FC<{ tab: Tab; contentJs: string; index: number }> = memo(
  ({ tab, contentJs, index }) => {
    ...
    useEffect(() => {
      ...
      webview.addEventListener('did-navigate', (e) => {
        ...
        console.log('- did-navigate', tab.id, e.url)
        setPageUrl(e.url)
      })
      webview.addEventListener('did-navigate-in-page', (e) => {
        console.log('- did-navigate-in-page', tab.id, e.url)
        setPageUrl(e.url)
      })
